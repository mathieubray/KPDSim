...
private:
	...
	KPDDonorAssignment donorAssignmentScheme;
	
	KPDTimeline timeline;

	KPDMatchFailure assumedMatchFailureScheme;
	KPDMatchFailure actualMatchFailureScheme;
...
public:
	...
	KPDDonorAssignment getDonorAssignmentScheme();
	
	KPDTimeline getTimeline();

	KPDMatchFailure getAssumedMatchFailureScheme();
	KPDMatchFailure getActualMatchFailureScheme();
...
KPDParameters::KPDParameters(){
	...
	donorAssignmentScheme = DONOR_ASSIGNMENT_PAIRED;

	timeline = TIMELINE_FIXED;
	
	assumedMatchFailureScheme = MATCH_FAILURE_RANDOM;
	actualMatchFailureScheme = MATCH_FAILURE_RANDOM;
	...
	while (!iFile.eof()){
		...
		while (!lineStream.eof()){
			...
			if (tokenOne.compare("#donorassignmentscheme") == 0){
				donorAssignmentScheme = KPDFunctions::stringToDonorAssignment(tokenTwo); //DONOR_ASSIGNMENT_PAIRED, DONOR_ASSIGNMENT_RANDOM
			}
			...
			if (tokenOne.compare("#assumedmatchfailurescheme") == 0){
				assumedMatchFailureScheme = KPDFunctions::stringToMatchFailure(tokenTwo); // MATCH_FAILURE_PRA_BASED, MATCH_FAILURE_RANDOM
			}
			if (tokenOne.compare("#actualmatchfailurescheme") == 0) {
				actualMatchFailureScheme = KPDFunctions::stringToMatchFailure(tokenTwo);
			}
...
void KPDParameters::printLog(){
	...
	kpdParametersLog << "Utility Based On: " << KPDFunctions::utilitySchemeToString(utilityScheme);
	...
	kpdParametersLog << "Donor Assignment Scheme: " << KPDFunctions::donorAssignmentToString(donorAssignmentScheme) << std::endl;
	
	kpdParametersLog << "Timeline: " << KPDFunctions::timelineToString(timeline) << std::endl;
	
	kpdParametersLog << "Assumed Match Failure Scheme: " << KPDFunctions::matchFailureToString(assumedMatchFailureScheme) << std::endl;
	kpdParametersLog << "Actual Match Failure Scheme: " << KPDFunctions::matchFailureToString(actualMatchFailureScheme) << std::endl;
	...
	if (assumedMatchFailureScheme == MATCH_FAILURE_RANDOM) {
	...
	if (actualMatchFailureScheme == MATCH_FAILURE_RANDOM) {
	...
	kpdParametersLog << "KPD Data File Name: " << fileKPDData << std::endl;
	kpdParametersLog << "HLA Frequency File Name: " << fileHLAFrequency << std::endl;
	kpdParametersLog << "HLA Dictionary File Name: " << fileHLADictionary << std::endl;
	kpdParametersLog << "Survival Parameters File Name: " << fileSurvivalParameters << std::endl;
...
KPDDonorAssignment KPDParameters::getDonorAssignmentScheme() {
...
KPDMatchFailure KPDParameters::getAssumedMatchFailureScheme(){
	return assumedMatchFailureScheme;
}

KPDMatchFailure KPDParameters::getActualMatchFailureScheme(){
