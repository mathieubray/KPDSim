...
public:
	...
	KPDRelation getRelationToCandidate();
	int getAge();
	bool getMale();
	KPDRace getRace();
	double getHeight();
	...
	void setRelationToCandidate(KPDRelation relation);
	void setAge(int age);
...
KPDDonor::KPDDonor() {
	...
	setRelationToCandidate(RELATION_UNSPECIFIED);
	setAge(40);
	setMale(true);
	setRace(RACE_WHITE);
	setHeight(72);
...
KPDDonor::KPDDonor(KPDBloodType bt, std::vector<std::string> hla, KPDRelation relation, int age, bool male, KPDRace race, double height, double weight, bool cigaretteUse) {
...
KPDDonor::KPDDonor(int mID, int dID, KPDBloodType bt, std::vector<std::string> hla, KPDRelation relation, int age, bool male, KPDRace race, double height, double weight, bool cigaretteUse) {
...
KPDRelation KPDDonor::getRelationToCandidate() {
...
void KPDDonor::setRelationToCandidate(KPDRelation relation) {
...
std::string KPDDonor::donorString() {
	...
	donorInfo << KPDFunctions::bloodTypeToString(donorBT) << ",";		// Blood Type
	donorInfo << KPDFunctions::relationToString(donorRelation) << ",";	// Relation to Candidate
	donorInfo << donorAge << ",";								// Age
	donorInfo << KPDFunctions::boolToMaleFemale(donorMale) << ",";// Gender
	donorInfo << KPDFunctions::raceToString(donorRace) << ",";		// Race
	donorInfo << donorHeight << ",";							// Height
	donorInfo << donorWeight << ",";							// Weight
	donorInfo << (donorWeight/donorHeight)/donorHeight << ",";	// BMI
	donorInfo << KPDFunctions::boolToYesNo(donorCigaretteUse);	// Cigarette Use
...
std::string KPDDonor::donorOutput(int space) {
	...
	donorInfo << indent.str() << "BT: " << KPDFunctions::bloodTypeToString(donorBT) << std::endl;
	...
	donorInfo << indent.str() << "Relation to Candidate: " << KPDFunctions::relationToString(donorRelation) << std::endl;
	donorInfo << indent.str() << "Age: " << donorAge << ", Sex: " << KPDFunctions::boolToMaleFemale(donorMale) << ", Race: " << KPDFunctions::raceToString(donorRace) << std::endl;
	donorInfo << indent.str() << "Height: " << donorHeight << ", Weight: " << donorWeight << ", BMI: " << (donorWeight / donorHeight) / donorHeight << std::endl;
	donorInfo << indent.str() << "Cigarette Use: " << KPDFunctions::boolToYesNo(donorCigaretteUse) << std::endl;
