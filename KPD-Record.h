...
void KPDRecord::selectNodesFixed(){
	...
	KPDDonorAssignment donorAssignmentScheme = kpdParameters->getDonorAssignmentScheme();
	...
	for (unsigned i = 1; i <= matchRunSchedule.size(); i++){
		...
		while (nPairings < nPairingArrivals){
			...
			if (donorAssignmentScheme == DONOR_ASSIGNMENT_RANDOM) {
				...
				std::vector<KPDDonor *> associatedDonors = kpdData->retrieveDonors(candidate->getMatchingID());
				...
				// Pairing
					...
					for (int k = 1; k <= numDonors; k++) {
						...
						if (allowableMatch(virtualCrossmatchResult)) {
							...
							KPDMatchFailure assumedMatchFailureScheme = kpdParameters->getAssumedMatchFailureScheme();
							KPDMatchFailure actualMatchFailureScheme = kpdParameters->getActualMatchFailureScheme();
