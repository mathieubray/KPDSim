...
private:
	...
	std::vector<std::vector<bool> > matchRunIncidenceMatrixReduced; // No Implicit Transplants
	...
	double nddActiveToInactive;
	double nddActiveToWithdrawn;
	double donorActiveToInactive;
	double donorActiveToWithdrawn;
	double candidateActiveToInactive;
	double candidateActiveToWithdrawn;
	...
	void getLRSPairsOnly(std::vector<std::vector<int> > & currentMatchRunArrangements);
	int getChildLRSPairsOnly(std::vector<int> &tree, std::vector<int> &level, std::vector<int> &nodes, std::vector<int> &pairs, std::vector<bool> &visited, int next, int childLevel);

	void getLRSWithNDDs(std::vector<std::vector<int> > & currentMatchRunArrangements, int numberOfNDDs);
...
KPDMatchRun::KPDMatchRun(KPDParameters * params, int currentIteration, int currentMatchRun, double currentTime,
	...
	nddActiveToInactive = params->getAssumedNDDActiveToInactive();
	nddActiveToWithdrawn = params->getAssumedNDDActiveToWithdrawn();
	donorActiveToInactive = params->getAssumedDonorActiveToInactive();
	donorActiveToWithdrawn = params->getAssumedDonorActiveToWithdrawn();
	candidateActiveToInactive = params->getAssumedCandidateActiveToInactive();
	candidateActiveToWithdrawn = params->getAssumedCandidateActiveToWithdrawn();
	...
	for (unsigned i = 1; i < nodeVector.size(); i++) {
		...
		if (nodeTransplantationStatus[i] == TRANSPLANT_NO && getNodeStatus(i) == STATUS_ACTIVE) {
...
void KPDMatchRun::collectLRSForCurrentMatchRun(std::vector<std::vector<int> > & currentMatchRunArrangements) {
	...
	if (matchRunNumberOfNDDs > 0) {
		getLRSWithNDDs(currentMatchRunArrangements, matchRunNumberOfNDDs);
	}

	getLRSPairsOnly(currentMatchRunArrangements);
}
...
void KPDMatchRun::getLRSPairsOnly(std::vector<std::vector<int> > & currentMatchRunArrangements) {
	...
	for (int i = 1; i <= numberOfPairs; i++) {
		...
		while (!bfsTree.empty()) {
			...
			// Get child
			child = getChildLRSPairsOnly(bfsTree, bfsLevel, currentMatchRunNodes, pairs, visited, next, childLevel);
...
int KPDMatchRun::getChildLRSPairsOnly(std::vector<int> &tree, std::vector<int> &level, std::vector<int> &nodes, std::vector<int> &pairs, std::vector<bool> &visited, int next, int childLevel) {
...
void KPDMatchRun::getLRSWithNDDs(std::vector<std::vector<int> > & currentMatchRunArrangements, int numberOfNDDs) {
				...
				if (childCanBeNDD == true) {
					...
					for (int i = 0; i < (int)listOfBFSTrees.size(); i++) {
						for (int j = 0; j < (int)listOfBFSTrees[i].size(); j++) {
							newLRS.push_back(currentMatchRunNodes[listOfBFSTrees[i][j]]);
						}
					}
...
int KPDMatchRun::getChildLRSWithNDDs(bool &childIsAdjacentToLowerLevels, std::vector<std::vector<int> > &trees, std::vector<std::vector<int> > &levels, std::vector<int> &inducedLevels,
...
double KPDMatchRun::calculateExpectedUtility(std::vector<int> & arrangement) {
	...
	if (timeline == TIMELINE_FIXED) {
		nddAssumedProbability = 1 - (nddActiveToInactive + nddActiveToWithdrawn);
		donorAssumedProbability = 1 - (donorActiveToInactive + donorActiveToWithdrawn);
		candidateAssumedProbability = 1 - (candidateActiveToInactive + candidateActiveToWithdrawn);
	}
	else {
		nddAssumedProbability = exp((-1)*(nddActiveToInactive + nddActiveToWithdrawn)*processingTime);
		donorAssumedProbability = exp((-1)*(donorActiveToInactive + donorActiveToWithdrawn)*processingTime);
		candidateAssumedProbability = exp((-1)*(candidateActiveToInactive + candidateActiveToWithdrawn)*processingTime);
	}
