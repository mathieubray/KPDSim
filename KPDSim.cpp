...
void printOutput(KPDOptimizationScheme scheme, KPDSimulation * kpdSimulation){			
	...
	if (kpdParameters->outputArrangementList()) {
		updateFile(kpdSimulation->getOutputArrangementList(), scheme, OUTPUT_ARRANGEMENT_LIST);
	}
	if (kpdParameters->outputTransplantList()) {
		updateFile(kpdSimulation->getOutputTransplantList(), scheme, OUTPUT_TRANSPLANT_LIST);
	}
	if (kpdParameters->outputSimulationLog()){
		updateFile(kpdSimulation->printLog(), scheme, OUTPUT_SIMULATION_LOG);
	}
}
